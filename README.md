## Hi, I'm Shad 

I am a Life Sciences w Cs student exploring Python,ML and computational tools for biology.


### 🌱 Currently
- Learning ML with Python for scientific applications
- Building small lab-focused tools
- Interested in bioinformatics & data analysis

### 🛠 Tech Stack
- Python, SQL
- Flask
- Git, GitHub
- Biochemistry & lab data analysis

### 🔬 Projects
- Protein structure database : https://github.com/Shad921z/Protein-Structure-Database
- glucose estimator and analyzer : https://github.com/Shad921z/Glucose-Estimator

### Others
- 60 projects in python : https://github.com/Shad921z/60-python-projects

### Contributions
- 1st contribution: https://github.com/Shad921z/first-contributions

### Portfolio
-> https://shad921z.vercel.app/

### 📫 Connect
- LinkedIn: https://linkedin.com/shadkhan921z
- Email: shadkhan921z@gmail.com
